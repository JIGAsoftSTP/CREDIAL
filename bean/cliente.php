...
function listCliente(){
    ...
    while ( $valor = $call->getValors()){
        $character = str_split(str_replace(" ", "", strtoupper($valor["NAME"])));
        $letra = (count($character)>0) ? $character[0]: "*";
        $count = ( (!array_key_exists($letra, $resut)) ? 0 : count($resut[$letra]));
        $resut[$letra][ $count ] = $valor;
    }
