...
function carregarCliente() {
    var carregou = false;
    // if(clientes[selectCharacter].length > 700) addTable +=  (per/100*clientes[selectCharacter].length);
    // else
    {
        addTable = clientes[selectCharacter].length;
        i=0;
    }
    for (var ff = i; ff < clientes[selectCharacter].length && ff < addTable ; ff++) {
        var client = clientes[selectCharacter][ff];
...
function credito(a) {
    $("#cred-cli-nif").text(clientes[selectCharacter][a]["NIF"]+" - ");
    var lastName = clientes[selectCharacter][a]['SURNAME'].split(" ");
    $("#cred-cli-comName").text(clientes[selectCharacter][a]['NAME']+" "+lastName[lastName.length-1]);
    nifClient = clientes[selectCharacter][a]["NIF"];
...
function inforCiente(b) {
    $.ajax({
        url: "./bean/cliente.php",
        type: "POST",
        data: {"intensao": "loadStatusClient", "nifCliente": clientes[selectCharacter][b]["NIF"]},
        dataType: "json",
        success: function (e) {
...
$(".alphabet span").click(function () {
    ...
    mCount = 5;
    addTable = 0;
    selectCharacter = $(this).attr("value");
    $("#tableCliente").empty();
    carregarCliente();
});
